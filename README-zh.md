...
- **Updates:**
  - 修复了无法修改预设位置 1 和预设位置 2 的问题。
  - 优化了实时控制的操作逻辑；现在，在激活实时控制之前，系统将首先使用 MoveJ 移动到默认的预设位置 1。

### v0.257

- **Updates:**
  - 更改了机械臂的角度限制，使之与最新的硬件限制相匹配。
  - 优化了反向工作空间的逆向运动学计算。

### v0.256

- **Updates:**
  - 新增任务：MoveC。这个新任务允许机械臂从当前位置移动到笛卡尔坐标系中的目标位置。与 MoveL 不同，MoveC 不要求移动路径必须是直线，这样放宽了限制。这种灵活性使得机械臂能够在工作空间的边缘位置高效操作，即使在逆向运动学（IK）解不连续或接近奇异点的区域也能顺利工作，增强了其在复杂环境中的操作多样性和有效性。
